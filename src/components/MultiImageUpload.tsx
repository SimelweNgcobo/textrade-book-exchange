...
const MultiImageUpload = ({
  ...
  const validateFile = (file: File): boolean => {
    ...
    if (!file.type.startsWith("image/")) {
      toast.error(
        `${file.name} is not a valid image file. Please select a PNG, JPG, or WEBP image.`,
      );
      return false;
    }
  ...
  const handleFileUpload = async (
    ...
    try {
      ...
      console.log(
        `Uploading file: ${fileName}, Size: ${(file.size / 1024 / 1024).toFixed(2)}MB`,
      );
  ...
  const imageSlots = [
    ...
    {
      label: "Inside Pages",
      description: "Photo of table of contents or sample pages",
    },
  ];
  ...
  return (
    <div className={`space-y-6 ${className}`}>
      <div>
        <h3
          className={`${isMobile ? "text-base" : "text-lg"} font-medium mb-2`}
        >
            ...
            return (
              <div key={index} className="space-y-2">
                ...
                <Card className="border-2 border-dashed border-gray-300 hover:border-gray-400 transition-colors">
                  <CardContent
                    className={`p-${isMobile ? "4" : "6"} ${isMobile ? "min-h-[180px]" : "min-h-[200px]"} flex flex-col justify-center`}
                  >
                    {hasImage ? (
                        ...
                        >
                          {isCurrentlyUploading ? (
                            ...
                            <>
                              ...
                              {isMobile ? "Take/Upload Photo" : "Upload Image"}
                            </>
                          )}
                        </Button>
        ...
        <p
          className={`${isMobile ? "text-xs" : "text-sm"} text-gray-600 mt-4 text-center`}
        >
          All three photos are required to create your listing. This helps
