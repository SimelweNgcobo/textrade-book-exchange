...
import { EmailVerificationService, VerificationParams } from '@/services/emailVerificationService';
  ...
  useEffect(() => {
    const handleVerification = async () => {
      try {
        ...
        const params = EmailVerificationService.extractParamsFromUrl(searchParams);
        ...
        const result = await EmailVerificationService.verifyEmail(params, window.location.href);
        ...
        if (result.success) {
          ...
          const errorMessage = EmailVerificationService.getFormattedErrorMessage(result);

          // If no verification parameters found, show debug mode
          if (!params.token_hash && !params.token && !params.code && !params.error_code) {
            setStatus('debug');
            setMessage('No verification parameters found in URL. Please see debug information below.');
          } else {
            setStatus('error');
                  ...
                  <Card className="mb-6 text-left">
                    <CardContent className="p-4">
                      ...
                      <div className="text-xs text-gray-600 space-y-1">
                        ...
                        <div>Token Hash: {debugInfo.token_hash ? 'present' : 'none'}</div>
                        <div>Legacy Token: {debugInfo.token ? 'present' : 'none'}</div>
                        <div>Code: {debugInfo.code ? 'present' : 'none'}</div>
                        {debugInfo.error_code && <div>Error Code: {debugInfo.error_code}</div>}
                      </div>
                    </CardContent>
                  </Card>
                )}
            ...
            {status === 'debug' && (
              <>
                ...
                {debugInfo && (
                  <Card className="mb-6 text-left">
                    <CardContent className="p-4">
                      <h4 className="font-semibold mb-2">Email Verification Debug Info:</h4>
                ...
                <div className="space-y-6">
                  ...
                  <div className="space-y-3">
                    <p className="text-sm text-gray-600">
                      Please share the debug information above with support if you need help.
                    </p>
                    <Button
