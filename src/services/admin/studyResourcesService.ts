...
export const studyResourcesService = {
  ...
  createStudyResource: async (data: StudyResourcePayload): Promise<any> => {
    ...
    await new Promise(resolve => setTimeout(resolve, 500));
    return { ...data, id: Date.now().toString(), createdAt: new Date().toISOString(), views: 0, likes: 0 };
  },
  updateStudyResource: async (id: string, data: Partial<StudyResourcePayload>): Promise<any> => {
  ...
  updateStudyTip: async (id: string, data: Partial<StudyTipPayload>): Promise<any> => {
